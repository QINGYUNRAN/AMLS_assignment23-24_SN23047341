...
def DecisionTree(x_train, x_test, y_train, y_test):
    if os.path.exists('checkpoint/decisiontree_best_model.pkl'):
        best_model = joblib.load('checkpoint/decisiontree_best_model.pkl')
        y_pred = best_model.predict(x_test)
        accuracy = accuracy_score(y_test, y_pred)
        decisiontree_params = joblib.load(r"checkpoint/decisiontree_best_params.pkl")
        ...
        joblib.dump(random_search.best_params_, 'checkpoint/decisiontree_best_params.pkl')
        joblib.dump(random_search.best_estimator_, 'checkpoint/decisiontree_best_model.pkl')
    return random_search, random_search.best_params_, validation_score

def RandomForest(x_train, x_test, y_train, y_test):
    if os.path.exists('checkpoint/randomforest_best_model.pkl'):
        best_model = joblib.load('checkpoint/randomforest_best_model.pkl')
        y_pred = best_model.predict(x_test)
        accuracy = accuracy_score(y_test, y_pred)
        randomforest_params = joblib.load(r"checkpoint/randomforest_best_params.pkl")
        ...
        joblib.dump(random_search.best_params_, 'checkpoint/randomforest_best_params.pkl')
        joblib.dump(random_search.best_estimator_, 'checkpoint/randomforest_best_model.pkl')
        return random_search, random_search.best_params_, validation_score

def my_SVC(x_train, x_test, y_train, y_test):
    if os.path.exists('checkpoint/SVC_best_model.pkl'):
        best_model = joblib.load('checkpoint/SVC_best_model.pkl')
        y_pred = best_model.predict(x_test)
        accuracy = accuracy_score(y_test, y_pred)
        svc_params = joblib.load(r"checkpoint/SVC_best_params.pkl")
        ...
        joblib.dump(random_search.best_params_, 'checkpoint/SVC_best_params.pkl')
        joblib.dump(random_search.best_estimator_, 'checkpoint/SVC_best_model.pkl')
        return random_search, random_search.best_params_, validation_score


def my_adaboost(x_train, x_test, y_train, y_test):
    if os.path.exists('checkpoint/adaboost_best_model.pkl'):
        best_model = joblib.load('checkpoint/adaboost_best_model.pkl')
        y_pred = best_model.predict(x_test)
        accuracy = accuracy_score(y_test, y_pred)
        adaboost_params = joblib.load(r"checkpoint/adaboost_best_params.pkl")
        ...
        joblib.dump(random_search.best_params_, 'checkpoint/adaboost_best_params.pkl')
        joblib.dump(random_search.best_estimator_, 'checkpoint/adaboost_best_model.pkl')
        return random_search, random_search.best_params_, validation_score
...
def plot_depth_score(x_train, y_train):
    ...
    plt.ylabel('accuracy')
    plt.grid()
    plt.savefig('image_output/max_depth_accuracy.pdf', format='pdf')
...
def plot_n_forest(x_train, y_train):
    ...
    plt.grid()
    plt.savefig('image_output/max_depth_forest.pdf', format='pdf')
...
def plot_SVC(x_train, y_train):
    ...
    for c in np.arange(1, 51):
        ...
        scores.append(np.mean(cv_scores))
    plt.figure()
    plt.plot(np.arange(1, 51)*0.1, scores, marker='o')
    plt.xlabel('C')
    plt.ylabel('Accuracy')
    plt.title('Effect of C on SVC Accuracy')
    plt.grid()
    plt.savefig('image_output/C_SVC.pdf', format='pdf')
...
def plot_adaboost(x_train, y_train):
    ...
    plt.grid()
    plt.savefig('image_output/adaboost.pdf', format='pdf')
