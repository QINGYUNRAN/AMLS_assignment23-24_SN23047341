...
class SolutionB:
    def __init__(self, config):
        ...
        self.data_path_B = os.path.join(base_dir, '..', 'Datasets', 'PathMNIST', 'pathmnist.npz')
    ...
    def setup_dataloader(self):
        data_a = np.load(self.data_path_B)
    ...
    def setup_model(self):
        self.model = my_net(input_dim=3, num_classes=9, dropout_rate=self.config['dropout_rate']).to(self.device)
    ...
    def run(self):
        if self.config["retrain_flag"]:
            self.best_model, self.train_loss_history, self.valid_loss_history = train(
                ...
                self.config['num_epochs'], self.config['patience'],
    ...
    def params_search(self):
        def objective(trial):
            ...
            model = my_net(input_dim=3, num_classes=9, dropout_rate=dropout_rate).to(self.device)
    ...
    def check_balance(self):
        ...
        def check_balance(class_counts, total_samples, threshold=0.02):
            for count in class_counts.values():
                if count < threshold * total_samples:
                    return False
            return True
...
def main():
    config = {
        ...
        "patience": 10,
        "batch_size": 32,
        "learning_rate": 6.130419127136388e-05,
        "dropout_rate": 0.6215811268016947,
        "weight_decay": 3.734186103804895e-07,
        "clip_value": 3.9099035958216297,
        "retrain_flag": True,
        "check_balance": True,
        "params_search": True,
